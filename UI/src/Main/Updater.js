...
autoUpdater.on('error', (error) => {
    ...
    if (version !== null) {
        dialog.showMessageBox({
            ...
            message: "An error occured while attempting to auto-update. DDCut must be updated manually. You'll now be directed to the download page.\n",
            buttons: ["Ok"]
        }, e => {
            shell.openExternal("https:/github.com/Defdist/DDCut/releases/latest");
        });
    }
});
...
autoUpdater.on('update-downloaded', (event, releaseNotes, releaseName) => {
    ...
    var message = 'A new version of DDCut is available. Would you like to update now?\n';
    if (version !== null) {
        message += "\nVersion: " + version;
    }
