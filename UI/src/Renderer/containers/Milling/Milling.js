...
import {Button, Grid, IconButton, LinearProgress, Typography, CircularProgress} from '@material-ui/core';
...
class Milling extends React.Component {
    constructor() {
        ...
        this.state = {
            ...
            millingProgress: -1,
            showAlert: false,
			promptClearWCS: ipcRenderer.sendSync("Ghost::HasNonzeroWCS")
                && ipcRenderer.sendSync("Ghost::AllowWcsClearPrompt"),   // Possibly prompt to clear WCS
            wcsValueCheckFailedMessage: ipcRenderer.sendSync("Ghost::WcsValueCheck"),    // Empty string means no issue (no check needed or check succeeded)
            firmwareMinVersionMet: ipcRenderer.sendSync("Firmware::FirmwareMeetsMinimumVersion"),
            ddcutMinVersionMet: ipcRenderer.sendSync("Firmware::DDCutMeetsMinimumVersion", version),
            alertTitle: "",
            alertMessage: "",
            status_loop: false,
            paused: false,
    ...
    saveEdits() {
        ...
        ipcRenderer.send("Jobs::SetStepValues", {
                ...
                prompt: "Please download the latest version of DDCut at Ghostgunner.net/downloads",
    ...
    onClickChangeJob() {
        this.setState({
            availableJobs: ipcRenderer.sendSync("File::GetExistingJobs"),
    ...
    pickNewFile() {
        ...
        ipcRenderer.on("InvalidDDFile", (event, filename, error) => {
            this.setState({alertMessage: "DD file error: " + error, showAlert: true});
        });
    ...
    render() {
        ...
        function getMillingInProgressDisplay(milling) {
            if (milling.state.millingProgress >= 0) {
                return (
                    <React.Fragment>
                        <Typography variant='h4' color="secondary">{milling.state.millingProgress}%</Typography>
                    ...
                    return (
                        <>
                            <Button
                                ...
                                variant="contained"
                                color='error'
                                style={{ marginTop: '15px', width: '125px', height: '60px', fontSize: '22px', backgroundColor: 'blue', color: 'white' }}
                            >
                                Feed Hold/Resume
                            </Button>
                            <Button
        ...
        return (
			<React.Fragment>
                ...
                <Alert
                    open={ !this.state.ddcutMinVersionMet }
                    message="This project requires a more recent DD Cut than your current version. Unexpected behavior may occur."
                    yesNo={false}
                    onOk={(event) => { this.setState({ ddcutMinVersionMet: true }) } }
                    onCancel={(event) => { this.setState({ ddcutMinVersionMet: true }) } }
                    title="Minimum DD Cut Version Check Failed"
                />

                <Alert
                    open={this.openImageFailAlert()}
                    message="DDCut failed to load the image in this step. Please check the file path and try again."
                    yesNo={false}
                ...
                <Alert
                    ...
                    It is advised that you clear these values unless you specifically intend this behavior. Would you like DD Cut to reset the
                    work coordinates for you?"
                    yesNo={true}
                    onOk={(event) => {
                        ipcRenderer.send('Ghost::ClearG54ThroughG58');
                ...
                <Grid id={'milling-container'} container spacing={2} className={classes.millingStyle}>
                    <Grid item xs={3} id="steps" className={classes.stepsList}>
                        <Button style={{ marginTop: '5px', flex: 1 }} onClick={(event) => { onClickBack(this) }}>
                            <img
                                style={{ height: '16px' }}
                                src={path.join(__dirname, app.milling.back_to_main)}
                            />
                        </Button>
                        <Button style={{ marginTop: '5px', color: '#069076', fontFamily: ["Lato", "sans-serif"], fontSize: '14px', textTransform: 'none'}} onClick={this.onClickChangeJob} >
                            Change Job
                        </Button>
                        <div style={{ maxHeight: '50.4px', marginBottom: '8px'}}>
                            <div style={{ color: '#ffffff', fontFamily: ["Lato", "sans-serif"], fontSize: '14px', maxHeight: '17px', overflow: 'hidden' }}>
                                File: <span style={{ color: '#069076'}}>{this.state.fileName}</span>
                            </div>
                            <div style={{ maxHeight: '33.6px', overflow: 'hidden', color: '#ffffff', fontFamily: ["Lato", "sans-serif"], fontSize: '14px' }}>
                                Job: <span style={{ color: '#069076'}}>{this.state.jobName}</span>
                            </div>
                        </div>
                        <StepList 
                        ...
                        <Button color="secondary" disabled={!this.state.editMode} className={classes.addStep} style={{ marginTop: '-4px' }} onClick={this.handleAddStep.bind(this)}>Add Step</Button>
                    ...
                    <Grid item xs={4} id="middle_section" className={classes.middle}>
