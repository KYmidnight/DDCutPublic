...
{
	DD_LOG("String version - path: " + path);
	if (m_unzFile == NULL)
	{
...
void Archive::WriteZipFile(const std::string& path, const std::map<std::string, std::string> filesContents)
{
	...
	std::string tempPathString = p.parent_path().string() + "/tempZip.dd";
	DD_LOG("tempPathString: ");
	DD_LOG(tempPathString.c_str());
	...
	do {
		...
		for (const auto& editedFile : filesContents)
		{
			...
			DD_LOG("editedFile.first: " + editedFile.first);
			DD_LOG("filename_in_zip: " + s);
	...
	{
		DD_LOG("unzClose Successful");
	}
	else
	{
		DD_LOG("unzClose failed!");
	}
	...
	{
		DD_LOG("Failed To DELETE");
	}

    // Rename the temporary zip file to the original zip file's name
    rename(tempPathString.c_str(), path.c_str());
	DD_LOG("Reopening Orig Zip");
	m_unzFile = unzOpen(path.c_str());
	DD_LOG("Returning from WriteZipFile");
	return;
}

void Archive::CreateNewDDFile(const std::string& fileName, const std::string& path)
{
	m_path = path + "\\" + fileName + ".dd";
	int status;
	boost::filesystem::path p(path);
	std::string tempString = p.string() + "/" + fileName + ".dd";
    ...
    if (zipf != NULL) {
        ...
        for(const std::string &file : filesToAdd) {
			...
			if (file == "manifest.yml") 
			{
				...
				manifestPlaceHolderText += "      step_text: Please download the latest version of DDCut at Ghostgunner.net/downloads"; 
