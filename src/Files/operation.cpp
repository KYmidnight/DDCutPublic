...
Operation::Operation(const std::string manifest)
{
	...
	m_title = "Step Name";
	m_markdown = "\nStep Text";
	m_prompt = "Please download the latest version of DDCut at Ghostgunner.net/downloads";
}
...
bool Operation::Load(const std::shared_ptr<DDFile>& pDDFile)
{
	...
	if (!m_loaded)
	{
		...
		if (!GetFile().empty())
		{
			...
			try {
				pDDFile->ReadFile(GetFile(), vs);
			}
			catch (const std::exception& exception)
			{
				DD_LOG("Failed to read gcode file!");
		...
		if (!GetImage().empty())
		{
			...
			pDDFile->ReadFile(GetImage(), m_imageBytes);
		}
...
std::string Operation::GetImageBase64() const
{
	...
	DD_LOG("Sha256 HASH: " + sha256(tempStr));
	return ret;
}
