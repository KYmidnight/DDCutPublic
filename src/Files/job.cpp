...
Job::Job(const YAMLObject& jobYAML, Archive& fileContext)
{
    ...
    for (size_t i=0; i < children.size(); ++i)
	{
        if (children[i].getChildren().size() > 0)
		{
			...
			else if (value == "min_fw_version")
			{
			...
			else if (value == "min_ddcut_version") {
				m_minimumDDCutVersion = child.getValue();
			}
				...
				for (auto& chunk: chunks) {
					try {
						...
						DD_LOG("Error parsing manifest file with chunk: " + chunk);
					}
				}
...
void Job::AddOperation(const int stepIndex)
{
    ...
    DD_LOG("AddNewOperation - prevStepManifest: " + prevStepManifest);
