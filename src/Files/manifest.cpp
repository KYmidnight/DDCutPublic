...
void Manifest::LoadManifest(Archive& archive)
{
	...
	if (files.empty()) { throw DDException(DDException::BAD_MANIFEST, "empty manifest"); }
...
void Manifest::AddNewJob(const std::string& jobName, const std::string& jobDescription, const int& jobIndex, Archive& archive)
{
	...
	ls.emplace_back("      step_text: Please download the latest version of DDCut at Ghostgunner.net/downloads");
