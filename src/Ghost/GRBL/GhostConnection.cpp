...
GhostConnection::GhostConnection(
    ...
    m_pWriter(std::make_unique<GhostWriter>(pState, pSerial, m_pFeedRate)),
...
GhostConnection::Ptr GhostConnection::Connect(const GhostGunner& ghostGunner)
{
    MILL_LOG("Connecting to ghost...");
    if (ghostGunner.GetPath().length() == 0) {
        throw GhostException(GhostException::FAILED_OPEN, "Path Empty");
    }

    auto pState = std::make_shared<ConnectionState>();
    auto pConnection = std::shared_ptr<GhostConnection>(new GhostConnection(
        ghostGunner,
        pState,
        std::make_shared<SerialConnection>(ghostGunner.GetPath(), pState)
    ));
...
bool GhostConnection::Reset(bool sendSoftResetToGRBL)
{
    VerifyConnected();
...
void GhostConnection::ProbeReset()
{
    VerifyConnected();
...
void GhostConnection::EmergencyStop()
{
...
void GhostConnection::ExecuteCommand(const GCodeLine& gcode, const bool isManualEntry) {
    VerifyConnected();
    DD_LOG_SYNC("Executing: " + gcode.GetOriginal() + " => " + gcode.GetCleaned());
    auto lock = lock_guard{ m_mutex };
    ...
    if (!m_pState->IsLocked() && !m_pState->IsTimedOut() && !m_pState->IsAlarm()) {
        ...
        DD_LOG_SYNC("Failed to write: " + gcode.GetCleaned());
        DD_LOG_F("Locked: %d - TimedOut: %d - Alarm: %d", m_pState->IsLocked(), m_pState->IsTimedOut(), m_pState->GetAlarm());
        if (!gcode.GetInjectedCommand()) {
            GhostDisplayManager::AddLine(ELineType::ERR, "Failed to write \"" + gcode.GetOriginal() + "\"! The machine may be locked or the emergency stop may be engaged.");
        }
    }
}

void GhostConnection::ExecuteProgram(const std::vector<GCodeLine>& gcodes, const bool isManualEntry)
{
    auto lock = lock_guard{ m_mutex };
    MILL_LOG("BEGIN");
    VerifyConnected();
...
void GhostConnection::ExecuteRealtime(const char command)
{
    VerifyConnected();
...
void GhostConnection::ExecuteLine(const GCodeLine& line, const bool useBuffer, const bool manualEntry)
{
...
bool GhostConnection::TryProbeOperation(const GCodeLine& line) {
...
void GhostConnection::RestorePreprobeState() {
    try {
...
const GhostSettings& GhostConnection::GetSettings(const bool updateFirst)
{
...
void GhostConnection::ReadResponse(const bool blocking)
{
    while (true) {
        VerifyConnected();
        sleep_for(5ms);
        ...
        if (pLine != nullptr) {
            ...
            throw GhostException(GhostException::SOFTWARE_ESTOP);
        }
...
string GhostConnection::ReadResponseWithoutProcessing() {
    sleep_for(5ms);
    auto pLine = m_pSerialConnection->ReadLine();
...
vector<pair<string, Point3>> GhostConnection::GetOffsets() noexcept {
...
Point3 GhostConnection::GetLastProbedPoint() {
    lock_guard lock{ m_probePointLock };
    return m_lastProbedPoint;
}

void GhostConnection::SaveProbeResult(const std::string& resultStr) {
...
Point3 GhostConnection::GetCurrentPosition() {
    auto pos = Point3{};
    auto pStatus = m_pState->GetRealTimeStatus();
...
Vector3 GhostConnection::GetCurrentWCS() {
    auto wcs = Vector3{};
    auto pStatus = m_pState->GetRealTimeStatus();
...
void GhostConnection::TransformZAndWriteLine(const GCodeLine& line) {
