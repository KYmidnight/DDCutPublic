...
public:
    /* PARSERS */
    static RealTimeStatus Parse(const std::string& input, const uint8_t wcs, const Measurement::Unit unit, const uint8_t parserUnits)
    {
        ...
        for (size_t i = 5; i < fields.size(); i++) {
            ...
            if (field.first == "W") {
                DD_LOG("Adding work coordinates");
    ...
    static RealTimeStatus ParseV0(const std::string& input, const uint8_t wcs, const Measurement::Unit unit, const uint8_t parserUnits)
    {
        if (input.size() < 2 || input[0] != '<' || input[input.size() - 1] != '>') {
            throw std::exception();
        }
        ...
        DD_LOG("Status: " + statusAndCoords.first);
        DD_LOG("Coords: " + statusAndCoords.second);
        ...
        DD_LOG("Machine position str: " + machinePositionStr);
        ...
        DD_LOG("Work position str: " + workPositionStr);
        std::string workCoordinatesStr = StringUtil::SplitOnce(workPositionStr, ":").second;
        DD_LOG("Work coordinates str: " + workCoordinatesStr);
...
private:
    RealTimeStatus(
        ...
        LimitSwitchState&& limits,
        const uint8_t parserUnits) noexcept
