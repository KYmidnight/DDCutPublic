...
public:
	...
	void WriteChar(char val, const int timeout) {
		if (val == 24) { val = '|'; }
		DD_LOG("Writing: " + val);
		//GhostDisplayManager::AddLine(ELineType::WRITE, std::string{ val });
	...
	void WriteLine(const string& val, const int timeout) {
		...
		DD_LOG("Writing: " + val);
		GhostDisplayManager::AddLine(ELineType::WRITE, val);
	...
	unique_ptr<string> ReadLine() {
		if (m_GRBL_Mock.HasOutput()) {
			...
			if (msg[0] == '<' && status) {
				...
				GhostDisplayManager::AddLine(ELineType::READ, msg);
			}
...
struct SerialConnection::Imp
{
	...
	void Connect() {
		...
		if (!GetCommState(file, &dcb)) {
			CloseHandle(file);
			throw GhostException(GhostException::FAILED_OPEN, "GetCommState failed");
		}
		...
		if (!SetCommState(file, &dcb)) {
			CloseHandle(file);
			throw GhostException(GhostException::FAILED_OPEN, "SetCommState failed");
		}
		...
		if (!GetCommTimeouts(file, &timeout)) {
			CloseHandle(file);
			throw GhostException(GhostException::FAILED_OPEN, "GetCommTimeouts failed");
		}
		...
		if (!SetCommTimeouts(file, &timeout)) {
			CloseHandle(file);
			throw GhostException(GhostException::FAILED_OPEN, "SetCommTimeouts failed");
		}
		...
		while (Read(&t, 1) > 0)
		{
			...
			if (t == '\n')
			{
				...
				if (m_pState->GetBuffer().empty() || !m_pState->GetBuffer().front().GetInjectedCommand())
				{
					...
					{
						GhostDisplayManager::AddLine(ELineType::READ, line);
					}
