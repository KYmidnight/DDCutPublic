...
Comparands getComparands(const std::string& compareStatement)
{
    ...
    if (matchLeft.empty()) {
        throw GhostException(GhostException::M106_MALFORMED_TEST);
    }
...
void throwException(const std::string& errorMessage)
{
    ...
    throw GhostException(GhostException::M106_BOOL_TEST_FAIL, errorMessage);
} 

void sendSuccessMessage()
{
    // Dont show success message for release version
    //    GhostDisplayManager::AddLine(ELineType::READ, "M106 Check Passed");
