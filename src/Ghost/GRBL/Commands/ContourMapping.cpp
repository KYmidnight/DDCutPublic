...
void M111(const SerialConnection::Ptr& pSerial, const string& params) {
	GhostDisplayManager::AddLine(ELineType::WRITE, "Initiating M111");
	auto gunner = DDCutDaemon::GetInstance().GetConnection();
	if (!gunner) { 
		throw GhostException{ GhostException::NO_ACCESS, "Connection failed."};
	}
	auto prevPoint = gunner->GetLastProbedPoint();
	auto point = prevPoint;
	pSerial->WriteLine("G38.2" + params);
	while (point == prevPoint) {
		sleep_for(10ms);
		gunner->ReadResponse(true);
		point = gunner->GetLastProbedPoint();
	}
	...
	if (it != g_ContourMap.end()) {
		...
		pSerial->FlushReads();
		GhostDisplayManager::AddLine(ELineType::READ, "M111 Failed. Duplicate X & Y values.");
	}
	else {
		g_ContourMap.emplace_back(point);
		PrintContourMap();
		pSerial->FlushReads();
		GhostDisplayManager::AddLine(ELineType::READ, "M111 Complete");
	}
}

void M112() {
	if (g_ContourMap.size() < 3) {
		throw GhostException{ GhostException::M112_FAIL, "M112 Failed. Contour map needs at least 3 points." };
	}
	DD_LOG("Contour map enabled.");
	PrintContourMap();
	GhostDisplayManager::AddLine(ELineType::WRITE, "M112: Contour Adjustment Enabled");
	g_useContourMap.store(true, std::memory_order_release);
}

void M113() {
	DD_LOG("Contour map disabled.");
	GhostDisplayManager::AddLine(ELineType::WRITE, "M113: Contour Adjustment Disabled");
	g_useContourMap.store(false, std::memory_order_release);
}

void M114() {
	g_ContourMap.clear();
	DD_LOG("Contour map cleared.");
	GhostDisplayManager::AddLine(ELineType::WRITE, "M114: Contour Map Cleared");
...
Plane3 GetOffsetPlane(const Point3 point) {
	...
	// This retry mechanism allows DD Cut to retry if there is potentially another valid set of points.
	...
	throw std::runtime_error{ "No unique plane found. You must have at least three **non-colinear** points stored with M111 for DD Cut to perform slope adjustment." };
