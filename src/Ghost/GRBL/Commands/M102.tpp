...
// Summary : (Supported in DDcut Only) Evaluate the exprtk expression at time of execution, pulling variable values from the WCS registers,
...
// Note that exprtk does allow for assignment in its expressions, e.g. G56X := G54X + G55X or G57X += 10. DDCut will not actually execute
...
void M102(const string& args) {
  ...
  {
		GhostDisplayManager::AddLine(ELineType::ERR, "M102 Syntax Error - Invalid destination register. Must be G54-59(X,Y,Z)");
		throw GhostException(GhostException::M102_INVALID_DEST_REGISTER);
  }
  line.erase(0,4);
  ...
  if (offsets.size() == 0) 
  { 
		GhostDisplayManager::AddLine(ELineType::ERR, "M102 Failed - Could not get offsets");
		throw GhostException(GhostException::M102_OFFSETS_FAIL);
  }
  ...
  while(std::regex_search(varStart, line.cend(), var_matches, var_regex))
  {
    ...
    DD_LOG("Input: " + var_name + " => " + to_string(var_list[iterator]));
  ...
  line = new_line;
  DD_LOG("Parsed exprtk expression: " + line);
  ...
  {
		GhostDisplayManager::AddLine(ELineType::ERR, "M102 Syntax Error - Invalid exprtk expression");
		throw GhostException(GhostException::M102_INVALID_EXPRTK_EXPRESSION);
  }

  T result = expression.value();
  DD_LOG("Evaluated result: " + to_string(result));
  ...
  string dest_coord = dest_register.substr(3,1);
	...
	DD_LOG("END M102");
	GhostDisplayManager::AddLine(ELineType::READ, "M102 Complete");
}
