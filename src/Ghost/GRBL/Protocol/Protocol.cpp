...
void Protocol::ProcessResponse(const std::string& line)
{
	...
	if (line.size() != 0 && StringUtil::StartsWith(line, "[PRB:")) {
		auto conn = DDCutDaemon::GetInstance().GetConnection();
		if (conn != nullptr) {
	...
	if (StringUtil::StartsWith(line, "error:")) {
		...
		throw GhostException(GhostException::GRBL_ERROR, error_message);
	}
	...
	if (std::regex_match(line, GRBL::RXLOCKED) || line.find("[MSG:Reset") != std::string::npos) {
		DD_LOG("Machine locked.");
		m_pState->SetLocked(true);
		throw GhostException(GhostException::MACHINE_LOCKED, "Machine locked. Reset to continue.");
	}
	...
	if (StringUtil::StartsWith(line, "<")) {
		...
		try {
			RealTimeStatus status = m_version == GRBLVersion::GRBL1_0 ?
				RealTimeStatus::ParseV0(line, wcs, unit, parserUnits) :
				RealTimeStatus::Parse(line, wcs, unit, parserUnits);
			...
			DD_LOG_F("Failed to parse RealTimeStatus: %s", e.what());
		}
	...
	if (StringUtil::StartsWith(line, "ALARM:")) {
		...
		if (alarm_num == 4 || alarm_num == 5 || description == "Probe fail") {
			...
			throw GhostException(GhostException::ALARM, description);
		}
	}
