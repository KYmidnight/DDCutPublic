...
void Jog( GhostConnection::Ptr pConnection, const EJogDirection direction,
	...
	{
		DD_LOG_F("Exception thrown when trying to query status: %s", e.what());
		GhostDisplayManager::AddLine(ELineType::ERR, "Failed to query status. Is the machine still connected?");
		return;
	}
	...
	DD_LOG_F("Jog Command: %s", command.c_str());
	...
	if (pConnection->GetState().GetBuffer().fits(GCodeLine{ command }))
	{
		...
		{
			DD_LOG_F("Exception thrown when trying to jog: %s", e.what());
			return;
		}
	}
}
	...
	const bool continuous, const double distance_mm) noexcept
{
	...
	if (limitsOpt.has_value())
	{
		...
		if (!pStatus) {
			DD_LOG("Realtime status unavailable.");
		...
		return positiveDirection ? distance : (distance * -1.0);
	}
	else
	{
...
void StopJogging(GhostConnection::Ptr pConnection) noexcept {
	...
	{
		DD_LOG_F("Jogging error: %s", e.what());
	}
	catch (...)
	{
		DD_LOG("Jogging error: Unknown error");
	}
}
