...
MillingError GhostErrorHandler::GetError(const GhostConnection::Ptr& pConnection, const GhostException& exception)
{
    if (exception.getType() == GhostException::SOFTWARE_ESTOP) {
        DD_LOG("Resetting software E-stop error.");
    ...
    if (exception.getType() == GhostException::M101_FAIL) {
        DD_LOG("M101 Failure");
        DDLogger::Flush();
    ...
    if (exception.getType() == GhostException::TIMEOUT || pConnection->IsTimedOut()) {
        DD_LOG("Resetting to clear timeout error.");
        try { pConnection->Reset(); }
        catch (...) {
            // Do nothing.
        }
    ...
    if (exception.getType() == GhostException::M106_BOOL_TEST_FAIL) {
        DD_LOG("M106 Test Failed");
        return MillingError{ MillingError::Error, -1, "M106 Check Failed", exception.what()};
    }
    if (exception.getType() == GhostException::M106_MALFORMED_TEST) {
        DD_LOG("M106 Command Malformed");
