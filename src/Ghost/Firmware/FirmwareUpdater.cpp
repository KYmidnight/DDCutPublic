...
std::unique_ptr<FirmwareUpdater> FirmwareUpdater::Initialize(const GhostConnector::Ptr& pConnector)
{
...
FirmwareUpdater::~FirmwareUpdater()
{
	...
	DD_LOG_SYNC("FirmwareUpdater stopped");
}
...
void FirmwareUpdater::get_installed_firmware(const Lock&) const noexcept
{
	...
	if (m_pConnector->IsConnected() && pConnection != nullptr) {
		try {
			...
			DD_LOG_F("GetFirmwareVersion error: %s", e.what());
		}
		catch (...) {
			DD_LOG("GetFirmwareVersion error: Unknown error");
		}
	}
}
...
void FirmwareUpdater::get_available_firmware(const Lock&, const FirmwareVersion& installed) noexcept
{
	...
	try {
		...
		DD_LOG("Current firmware version: " + installed.GetYMD());
