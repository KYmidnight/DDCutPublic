...
void AVRDude::Execute32m1(const GhostConnection::Ptr& pConnection, const fs::path& hexFile)
{
	int status = -1;
	for (int i = 0; i < 2; i++) {
...
void AVRDude::Execute328p(const GhostConnection::Ptr& pConnection, const fs::path& hexFile)
{
	int status = -1;
	for (int i = 0; i < 2; i++) {
...
int AVRDude::Execute(const GhostConnection::Ptr& pConnection,
	...
	const std::string& programmer) const
{
	...
	DD_LOG("Executing Command: " + command);
#endif
	...
	DD_LOG(std::regex_replace(output, std::regex("\n"), "\r\n"));

	if (exit_status != 0) {
		DD_LOG_F("Command failed: %d", exit_status);
	}
