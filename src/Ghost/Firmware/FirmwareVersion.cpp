...
FirmwareVersion FirmwareVersion::Parse(const std::string& buffer)
{
	...
	if (line.find(',') != std::string::npos) {
		...
		const std::string ddVersion = StringUtil::Trim(parts[1]).substr(3);

		return FirmwareVersion(grblVersion, ddVersion);
	} else {
		// [grbl:1.1h GG:3A PCB:3B VFD:3A YMD:20200101]
		std::vector<std::string> parts = StringUtil::Split(line, " ");
		std::string grbl;
		std::string gg;
		std::string pcb;
		...
		return FirmwareVersion(grbl, ymd, gg, pcb, vfd);
	}
}
