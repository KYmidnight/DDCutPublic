...
public:
	...
	const std::string& GetChassis() const noexcept { return m_ggVersion; }
...
private:
	FirmwareVersion(const std::string& grbl, const std::string& ymd, const std::string& gg = "2A", const std::string& pcb = "2A", const std::string& vfd = "00")
		: m_grblVersion(grbl), m_ymd(ymd), m_ggVersion(gg), m_pcbVersion(pcb), m_vfdVersion(vfd) { }

	std::string m_grblVersion;
	std::string m_ymd;
	std::string m_ggVersion;
