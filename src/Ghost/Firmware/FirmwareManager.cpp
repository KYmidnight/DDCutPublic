	...
	SerialConnection& connection) const
{
	...
	auto iter = m_versionCache.find(ghostGunner.GetSerialNumber());
	...
	DD_LOG("Firmware Version returned: " + *pResponse);
	connection.FlushReads();

	// Parse GG's Response
	const FirmwareVersion version = FirmwareVersion::Parse(*pResponse);

	// Cache GG's Firmware Version
	m_versionCache.insert({ ghostGunner.GetSerialNumber(), version });
	...
	const URL& firmware328pURL) noexcept
{
	...
	{
		SetStatus(-1);
		DD_LOG_F("Exception thrown: %s", e.what());
	}
}
	...
	const URL& firmware328pURL)
{
	...
	auto version = GetFirmwareVersion(pConnection->GetGhostGunner(), *pConnection->GetSerial());
	auto iter = m_versionCache.find(pConnection->GetGhostGunner().GetSerialNumber());
	if (iter != m_versionCache.end())
	{
	...
	{
		DD_LOG("Failed while updating 32m1. Attempting to install 328p firmware.");
		GhostDisplayManager::AddLine(ELineType::READ, "Failed while updating 32m1. Attempting to install 328p firmware.");
...
void FirmwareManager::UpdateCustom(
	...
	GhostDisplayManager::AddLine(ELineType::READ, "Upload status: 99%");
	AVRDude::Execute328p(pConnection, fs::path{ firmware328pURL });
	SetStatus(100);
	GhostDisplayManager::AddLine(ELineType::READ, "Upload status: 100%");
}
