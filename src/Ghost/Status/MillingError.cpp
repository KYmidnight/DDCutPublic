...
static const std::unordered_map<int, AlarmCode> ALARM_CODES_CSV = {
	...
	{ALARM_CODE_TIMEOUT,{"Operation timed out.\n\nUnfortunately DDcut cannot continue. Please try again."}},
...
MillingError ErrorCodes::GetError(const int error, const tl::optional<GCodeLine>& last_command) noexcept
{
	...
	std::string description = command + " returned error " + std::to_string(error) + ": " + error_code.message + ": " + error_code.description +
		"\n\nIf this is a known good.dd file, then a data transmission error occurred. If this is development code, try enabling check code mode to verify all g-code syntax is valid."
		"\n\nUnfortunately g-code behavior after experiencing an error is undefined, so DDcut cannot continue. Please try again.";
