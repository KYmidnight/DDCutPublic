...
// Adaptor that translates calls from Electron front-end to DDCutDaemon API
...
{
	std::shared_ptr<DDFile> ddFile = DDCutDaemon::GetInstance().GetDDFile();
	std::string path = ddFile != nullptr ? ddFile->GetPath() : "";
...
napi_value SetDDFile(napi_env env, napi_callback_info info)
{
	...
	tl::optional<std::string> error = DDCutDaemon::GetInstance().SetDDFile(args[0].ToString().Get());
	return NapiString::Create(env, error.value_or("")).napi;
}

napi_value CreateNewDDFile(napi_env env, napi_callback_info info)
{
	std::vector<NapiValue> args = NapiArgs::GetArgs(env, info);
	std::string fileName = args[0].ToString().Get();
	std::string path = args[1].ToString().Get();
	DDCutDaemon::GetInstance().CreateNewDDFile(fileName, path);
	return NapiBool::Create(env, true).napi;
}

napi_value AddNewFileToDDFile(napi_env env, napi_callback_info info)
{
	bool result;
	std::vector<NapiValue> args = NapiArgs::GetArgs(env, info);
	...
	try
	{
		result = DDCutDaemon::GetInstance().AddNewFileToDDFile(filepath, fileType);
	}
...
napi_value ExtractAdditionalDdContentsIntoDirectory(napi_env env, napi_callback_info info) {
	std::vector<NapiValue> args = NapiArgs::GetArgs(env, info);

	std::string directory = args[0].ToString().Get();
	DD_LOG("Node: " + directory);
	const bool valid = DDCutDaemon::GetInstance().ExtractAdditionalDdContentsIntoDirectory(directory);
...
napi_value HasAdditionalContent(napi_env env, napi_callback_info info) {
	try {
		return NapiBool::Create(env, DDCutDaemon::GetInstance().HasAdditionalContent()).napi;
	}
	catch (...) {
...
napi_value IsValidDDFile(napi_env env, napi_callback_info info)
{
	...
	std::string ddFile = args[0].ToString().Get();
...
napi_value GetGhostGunnerStatus(napi_env env, napi_callback_info info)
{
	...
	const MillingStatus millingStatus = DDCutDaemon::GetInstance().GetMillingStatus(false);
	result.SetBool("milling", millingStatus.GetStatus() == EMillingStatus::inProgress);
	
	EGhostGunnerStatus connection_status = DDCutDaemon::GetInstance().GetGhostGunnerStatus();
	if (millingStatus.GetError().has_value() && millingStatus.GetError().value().IsEStop() 
		&& DDCutDaemon::GetInstance().EstopEngaged()) {	// Cross-check E-stop since our logic is a bit wonky
		connection_status = EGhostGunnerStatus::connectionFailed;
	}

	switch (connection_status)
	{
		case EGhostGunnerStatus::connected: DD_LOG("connected"); break;
		case EGhostGunnerStatus::connecting: DD_LOG("connecting"); break;
		case EGhostGunnerStatus::connectionFailed: DD_LOG("connectionFailed"); break;
		case EGhostGunnerStatus::notConnected: DD_LOG("noConnected"); break;
	}
...
{
	const auto& daemon = DDCutDaemon::GetInstance();
	const std::list<GhostGunner> availableGhostGunners = daemon.GetAvailableGhostGunners();

	NapiArray ghostGunnersArray = NapiArray::Create(env, availableGhostGunners.size());

	int i = 0;
	for (auto iter = availableGhostGunners.cbegin(); iter != availableGhostGunners.cend(); iter++) {
		const GhostGunner ghostGunner = *iter;

		NapiObject ghostGunnerObj = NapiObject::Create(env);
		ghostGunnerObj.SetString("serial_number", ghostGunner.GetSerialNumber());
		ghostGunnerObj.SetString("path", ghostGunner.GetPath());
		ghostGunnerObj.SetBool("selected", daemon.IsSelectedGhostGunner(ghostGunner));
		ghostGunnersArray.Set(i++, ghostGunnerObj.napi);
	}

	return ghostGunnersArray.napi;
}
...
{
	auto& daemon = DDCutDaemon::GetInstance();
	...
	if (pConnection != nullptr)
	{
		...
		GhostSettings settings = pConnection->GetSettings(false);
...
{
	const std::vector<Job> jobs = DDCutDaemon::GetInstance().GetJobs();
...
napi_value SelectJob(napi_env env, napi_callback_info info)
{
	...
	DDCutDaemon::GetInstance().SelectJob(
		args[0].ToInt32().Get()
	);
...
{
	return NapiBool::Create(env, DDCutDaemon::GetInstance().IsSubmanifestUsed()).napi;
}

napi_value GetAllSteps(napi_env env, napi_callback_info info)
{
	const std::vector<Operation::Ptr> steps = DDCutDaemon::GetInstance().GetAllSteps();
	...
	for (size_t i = 0; i < steps.size(); i++)
	{
		...
		DD_LOG("Return title: " + steps[i]->GetTitle());
...
napi_value GetStep(napi_env env, napi_callback_info info)
{
	...
	if (pOperation != nullptr)
	{
		...
		try {
			if (pOperation->Load(daemon.GetDDFile()))
			{
				...
				else
				{
					...
					for (size_t i = stepIndex + 1; i < steps.size(); i++)
					{
						...
						if (pStep->Load(daemon.GetDDFile()))
						{
							DD_LOG(pStep->GetTitle() + " " + pStep->GetUnskippable());
		...
		{
			stepObj.SetString("Error", "DDCut failed to load image from file path.");
		}
		return stepObj.napi;
	}
	else
...
napi_value AddNewOperation(napi_env env, napi_callback_info info)
{
	...
	DD_LOG("running AddNewOperation");
	daemon.AddNewOperation(stepIndex.Get());
	DD_LOG("Finished AddNewOperation");
	return NapiBool::Create(env, true).napi;
}
...
{
	bool writeInProgress = DDCutDaemon::GetInstance().GetWriteStatus();
...
napi_value StartMilling(napi_env env, napi_callback_info info)
{
	...
	const int32_t stepIndex = args[0].ToInt32().Get();
	const bool millingStarted = DDCutDaemon::GetInstance().StartMilling(stepIndex);
...
{ 
	...
	const bool millingStarted = DDCutDaemon::GetInstance().RunManualGCodeFile(filePath);
	return NapiBool::Create(env, millingStarted).napi;
}

napi_value GetMillingStatus(napi_env env, napi_callback_info info)
{
	const MillingStatus millingStatus = DDCutDaemon::GetInstance().GetMillingStatus(true);
...
{
	DDCutDaemon::GetInstance().EmergencyStop();
	return nullptr;
}
...
napi_value SetJogKeys(napi_env env, napi_callback_info info)
{
	...
	DDCutDaemon::GetInstance().SetJogKeys(jogKeys);
...
{
	return NapiBool::Create(env, DDCutDaemon::GetInstance().GetEnableSlider()).napi;
}

napi_value GetPauseAfterGCode(napi_env env, napi_callback_info info)
{
	return NapiBool::Create(env, DDCutDaemon::GetInstance().GetPauseAfterGCode()).napi;
}

napi_value GetMinFeedRate(napi_env env, napi_callback_info info)
{
	return NapiUInt32::Create(env, DDCutDaemon::GetInstance().GetMinFeedRate()).napi;
}

napi_value GetMaxFeedRate(napi_env env, napi_callback_info info)
{
	return NapiUInt32::Create(env, DDCutDaemon::GetInstance().GetMaxFeedRate()).napi;
}

napi_value GetFeedRate(napi_env env, napi_callback_info info)
{
	return NapiUInt32::Create(env, DDCutDaemon::GetInstance().GetFeedRate()).napi;
}

napi_value GetDisableLimitCatch(napi_env env, napi_callback_info info)
{
	return NapiBool::Create(env, DDCutDaemon::GetInstance().GetDisableLimitCatch()).napi;
}
...
napi_value UpdateSettings(napi_env env, napi_callback_info info)
{
	...
	DDCutDaemon::GetInstance().UpdateSettings(settings);

	return nullptr;
}

napi_value HasNonzeroWCS(napi_env env, napi_callback_info info) {
	return NapiBool::Create(env, DDCutDaemon::GetInstance().HasNonzeroWCS()).napi;
}

napi_value AllowWcsClearPrompt(napi_env env, napi_callback_info info) {
	return NapiBool::Create(env, DDCutDaemon::GetInstance().AllowWcsClearPrompt()).napi;
}

napi_value ClearG54ThroughG58(napi_env env, napi_callback_info info) {
	DDCutDaemon::GetInstance().ClearG54ThroughG58();
	return nullptr;
}

napi_value WcsValueCheck(napi_env env, napi_callback_info info) {
	return NapiString::Create(env, DDCutDaemon::GetInstance().WcsValueCheck()).napi;
}

napi_value FirmwareMeetsMinimumVersion(napi_env env, napi_callback_info info) {
	return NapiBool::Create(env, DDCutDaemon::GetInstance().FirmwareMeetsMinimumVersion()).napi;
}

napi_value DDCutMeetsMinimumVersion(napi_env env, napi_callback_info info) {
	std::vector<NapiValue> args = NapiArgs::GetArgs(env, info);
	std::string input = args[0].ToString().Get();

	return NapiBool::Create(env, DDCutDaemon::GetInstance().DDCutMeetsMinimumVersion(input)).napi;
}

napi_value SetFeedRate(napi_env env, napi_callback_info info)
{
	std::vector<NapiValue> args = NapiArgs::GetArgs(env, info);
	DDCutDaemon::GetInstance().SetFeedRate(args[0].ToInt32().Get());
	return nullptr;
}
...
napi_value UploadCustomFirmware(napi_env env, napi_callback_info info) {
	DD_LOG("Loading custom firmware...");
	std::vector<NapiValue> args = NapiArgs::GetArgs(env, info);

	auto path328p = args[0].ToString().Get();
	DD_LOG("path328p: " + path328p);
	auto path32m1 = std::string{ };
	try {
		path32m1 = args[1].ToString().Get();
		DD_LOG("path32m1: " + path32m1);
	}
	catch (...) {
		DD_LOG("Error parsing file path.");
	}
	
	const bool uploaded = DDCutDaemon::GetInstance().UploadCustomFirmware(path328p, path32m1);
...
{
	return NapiInt32::Create(env, DDCutDaemon::GetInstance().GetFirmwareUploadStatus()).napi;
}

auto FirmwareUpdateAvailable(napi_env env, napi_callback_info info) -> napi_value
{
	return NapiBool::Create(env, DDCutDaemon::GetInstance().FirmwareUpdateAvailable()).napi;
}

napi_value GetFirmwareVersion(napi_env env, napi_callback_info info)
{
	Json::Value json;
	auto version = DDCutDaemon::GetInstance().GetFirmwareVersion();
...
napi_value ShouldShowWalkthrough(napi_env env, napi_callback_info info)
{
	...
	const bool show = DDCutDaemon::GetInstance().ShouldWalkthroughDisplay(
...
napi_value SetShowWalkthrough(napi_env env, napi_callback_info info)
{
	...
	DDCutDaemon::GetInstance().SetShowWalkthrough(
		WalkthroughType::FromString(args[0].ToString().Get()),
		args[1].ToBool().Get()
	);
...
{
	return NapiString::Create(env, DDCutDaemon::GetInstance().GetLogPath()).napi;
}

napi_value LogString(napi_env env, napi_callback_info info) {
	try {
		DD_LOG("Writing front-end log string...");
		std::vector<NapiValue> args = NapiArgs::GetArgs(env, info);
		for (auto&& s : args) {
			DD_LOG(s.ToString().Get());
		}
	}
	catch (...) { }
	return nullptr;
...
{
	auto status = DDCutDaemon::GetInstance().GetStatus();
...
napi_value Jog(napi_env env, napi_callback_info info)
{
	...
	DDCutDaemon::GetInstance().Jog(jogDirection, continuous, distance_mm);
...
{
	DDCutDaemon::GetInstance().CancelJog();
...
napi_value ExecuteCommand(napi_env env, napi_callback_info info)
{
	...
	DDCutDaemon::GetInstance().ExecuteCommand(args[0].ToString().Get());
...
napi_value ExecuteRealtime(napi_env env, napi_callback_info info)
{
	...
	DDCutDaemon::GetInstance().ExecuteRealtime((uint8_t)args[0].ToUInt32().Get());
...
napi_value SetManualEntryMode(napi_env env, napi_callback_info info)
{
	...
	DDCutDaemon::GetInstance().SetManualOperationFlag(manualEntryMode);
...
{
	return NapiBool::Create(env, DDCutDaemon::GetInstance().InstallDrivers()).napi;
}

napi_value Shutdown(napi_env env, napi_callback_info info)
{
	DD_LOG_SYNC("Shutdown API called from node");
...
napi_value Init(napi_env env, napi_value exports)
{
	...
	std::vector<napi_property_descriptor> descriptors{
		...
		DECLARE_NAPI_METHOD("CreateNewDDFile", CreateNewDDFile),
		DECLARE_NAPI_METHOD("AddNewFileToDDFile", AddNewFileToDDFile),
		DECLARE_NAPI_METHOD("ExtractAdditionalContentsInto", ExtractAdditionalDdContentsIntoDirectory),
		...
		DECLARE_NAPI_METHOD("DDCutMeetsMinimumVersion", DDCutMeetsMinimumVersion),
