...
fs::path FileUtility::GetWorkingDirectory()
{
	...
	DD_LOG("Error getting current directory: #" + std::to_string(GetLastError()));
#endif
...
{
    DD_LOG("Setting working directory: " + newDirectory.string());
#ifdef _WIN32
    if (!SetCurrentDirectory(newDirectory.u8string().c_str()))
    {
        DD_LOG("Error setting current directory: #" + std::to_string(GetLastError()));
        throw std::exception();
    }
#endif
}
