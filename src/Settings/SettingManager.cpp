...
static const std::string CONFIG_FILE_NAME = "ghost.conf";
...
SettingManager::SettingManager()
{
	...
	if (settings.empty())
	{
		DD_LOG("Config file path does not yet exist. Creating it now.");

		UpdateSettings(std::list<Setting>{{"GhostGunner", "5.0"}});
	}
	else
	{
...
bool SettingManager::UpdateSettings(const std::list<Setting>& settings)
{
	...
	DD_LOG(std::to_string(m_settingsByKey.size()));
