...
This single script handles everything: system dependencies, nvm/Node.js, vcpkg upgrade, building the tcmalloc shim, compiling the C++ addon, and installing the Electron frontend.
...
| `-Wa,-mbig-obj` assembler error | x86_64 doesn't support this flag | This is fixed in the updated `FindFilesystem.cmake` |
| `DDUtil` link failure (tiny-process-library ABI) | C++11 string ABI mismatch between vcpkg and manual builds | Expected — DDUtil is a test binary, not needed |
...
| Error | Cause | Fix |
|-------|-------|-----|
| `tcmalloc: Attempt to free invalid pointer` | Running without the LD_PRELOAD shim | Add `LD_PRELOAD=path/to/tcmalloc_shim.so` |
...
| `SetDDFile(94)` interpreting `--force-device-scale-factor=0.8` as a filename | Known bug in dd file parsing | Cosmetic — doesn't affect functionality |

### Rebuilding After Source Changes

If you modify C++ source files, you only need to rebuild the addon (step 6), not the entire vcpkg dependencies:
...
If you modify CMakeLists.txt, rebuild from scratch:

```bash
rm -rf build && mkdir build && cd build
# ... repeat step 6 cmake commands
```

## Windows Build (Legacy)

For Windows builds, see the original [DDCut repo](https://github.com/DDCut/DDCut). The Windows build instructions in this file are outdated.
